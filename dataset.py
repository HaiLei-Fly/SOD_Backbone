...
def pad_resize_image(inp_img, out_img=None, target_size=None):
    ...
    :param out_img:一个 HxWxC 输入图像
    :param traget:目标调整尺寸
    :return:调整大小的图像
    """
    if out_img is None:
        if target_size is not None:
            temp_x = cv2.resize(inp_img, (target_size, target_size), interpolation=cv2.INTER_AREA)          
        return temp_x
    else:
        if target_size is not None:
            temp_x = cv2.resize(inp_img, (target_size, target_size))
            temp_y = cv2.resize(out_img, (target_size, target_size), interpolation=cv2.INTER_AREA)
...
class SODLoader(Dataset):
    """
    用于DUTS数据集的数据加载器(用于训练)
    """
    def __init__(self, mode='train', augment_data=True, target_size=352):
...
class PreDataloader(Dataset):
    ...
    用于测试数据集的数据加载器(用于测试)
    """
    def __init__(self, img_folder,target_size=352):
    ...
    def __getitem__(self, idx):
        ...
        """
        img = cv2.imread(self.imgs_folder[idx])
