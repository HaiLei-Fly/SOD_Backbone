...
class backbone(nn.Module):
    def __init__(self):
        ...
        self.up_conv_4 = DoubleConv(512, 512)
        self.up_conv_5 = DoubleConv(512, 512)
        self.out_conv = OutConv(64, 1)

        self.conv_5r = nn.Conv2d(512, 512, kernel_size=1, padding=0)
        self.bn_5r = nn.BatchNorm2d(512)  
    ...
    def forward(self, input):
        # VggNet_F1:176x176x64;VggNet_F2:88x88x128;VggNet_F3:44x44x256;VggNet_F4:22x22x512;VggNet_F5:11x11x512;
        VggNet_F1, VggNet_F2, VggNet_F3, VggNet_F4, VggNet_F5 = self.vggnet(input)
        # decoder_f5：11*11*512
        decoder_f5 = self.up_conv_5(VggNet_F5)
        # decoder_f5_r:22*22*512
...
class Model(nn.Module):
    ...
    def forward(self, input):
        # out_f：352*352*1, decoder_f1：176*176*64, decoder_f2：88*88*128, decoder_f3：44*44*256, decoder_f4：22*22*512
        out_f, decoder_f1, decoder_f2, decoder_f3, decoder_f4 = self.backbone(input)
        # decoder_f1：352*352*1
